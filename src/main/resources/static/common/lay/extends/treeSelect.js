 ...
 */
        ...
        var a = {
            init: function () {
                $.ajax({
                    ...
                    success: function (d) {
                        ...
                        if(!showCheckbox){
                            $(".layui-treeSelect .ztree li span.button.ico_open").hide();
                            $(".layui-treeSelect .ztree li span.button").hide();
                            $(".ztree li span").css("padding-left","10px");
                        } else {
                            $(".layui-input-inline .layui-treeSelect .ztree li span.button.switch").css("top", "-8px");
                        }
                    }
                });
                return a;
            ...
            configStyle: function () {
                ...
                if (style == undefined || style.folder == undefined || !style.folder.enable) {
                    $('#' + TREE_SELECT_ID+' li span.button.ico_open').hide();
                    $('#' + TREE_SELECT_ID +' li span.button.ico_close').hide();
                }
            },
     ...
     * @param filter
     */
    TreeSelect.prototype.reload = function (filter, url, where) {
        var treeObj = obj.treeObj(filter);
        $.ajax({
            url: url,
            type: treeObj.type,
            data: where,
            headers: treeObj.headers,
            dataType: 'json',
            success: function (d) {
                $.fn.zTree.init($('#' + treeObj.setting.treeId), treeObj.setting, d);
            }
        });
    };
