    ...
    .define(
        ...
        function (exports) {
            ...
            self.loadHtml = function (url, callback) {
                ...
                let resUrl=(url.indexOf(conf.base) === 0 ? '' : url.indexOf(conf.views) === 0 ? '' : conf.views) +
                    url +
                    conf.engine +
                    '?v=' +
                    conf.v;
                ...
                $.ajax({
                    ...
                    success: function (r) {
                        ...
                        if(resUrl.indexOf("http:")== 0){
                            let reg=/(\w+):\/\/([^/:]+)(:\d*)?/;
                            r=r.replace(/ctx/g,"'"+resUrl.match(reg)[0]+"/'");
                            r=r.replace(/'操作'/g,"'操作',hide: true");
                            r=r.replace(/action-more/g,"action-more febs-hide");
                            r=r.replace(/action-more/g,"action-more febs-hide");
                            for (let i=0;i<2;i++){
                                r=r.replace("layui-btn layui-btn-sm layui-btn-primary table-action","febs-hide");
                            }
                        }
                        callback({
                            html: r,
