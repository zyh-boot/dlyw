...
public class AccountServiceImpl extends ServiceImpl<AccountMapper, Account> implements IAccountService {
     ...
     */
    @Override
    public Account selectOne(Wrapper wrapper) {
     ...
     */
    @Override
    public List<Account> list(Account obj) {
     ...
     */
    @Override
    public IPage<Account> page(Account obj, QueryRequest query) {

        LambdaQueryWrapper<Account> queryWrapper = new LambdaQueryWrapper<>();

        if (StringUtils.isNotBlank(obj.getStartDate()) && StringUtils.isNotBlank(obj.getEndDate())) {
            queryWrapper.between(Account::getCreateDate, obj.getStartDate(), obj.getEndDate());
        }
        if(StringUtils.isNotBlank(obj.getKhName())){
             queryWrapper.like(Account::getKhName,obj.getKhName());
        }

        if(obj.getKhDegree()!=null){
            queryWrapper.eq(Account::getKhDegree,obj.getKhDegree());
        }
        if(obj.getKhStatus()!=null){
            queryWrapper.eq(Account::getKhStatus,obj.getKhStatus());
        }
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public int updateByWrapper(Account obj, Wrapper wrapper) {
    ...
    public int logicDel(Long id) {
        LambdaUpdateWrapper<Account> updateWrapper = new UpdateWrapper<Account>().lambda();
    ...
    public int batchDelLogic(String ids) {
        ...
        LambdaUpdateWrapper<Account> updateWrapper = new UpdateWrapper<Account>().lambda();
        updateWrapper.in(Account::getId, idLists);
        Account obj = new Account();
        obj.setState(Constant.STATE_0);
        return this.baseMapper.update(obj, updateWrapper);
    }
    ...
    public   Account  findAccountByAccandPwd(Account map) {
        return this.baseMapper.findAccountByAccandPwd(map);
    };
}
