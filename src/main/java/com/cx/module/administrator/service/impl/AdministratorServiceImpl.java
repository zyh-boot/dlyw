package com.cx.module.administrator.service.impl;
...
import com.cx.module.administrator.mapper.AdministratorMapper;
...
*/
@Override
public Administrator selectOne(Long id){
...
*/
@Override
public Administrator selectOne(Wrapper wrapper){
...
*/
@Override
public List<Administrator> list(Administrator obj){
...
*/
@Override
public IPage<Administrator> page(Administrator obj, QueryRequest query){

LambdaQueryWrapper<Administrator> queryWrapper=new LambdaQueryWrapper<>();

if (StringUtils.isNotBlank(obj.getStartDate()) && StringUtils.isNotBlank(obj.getEndDate())) {
queryWrapper.between(Administrator ::getCreateDate, obj.getStartDate(), obj.getEndDate());
}

Page<Administrator> page = new Page<>(query.getPageNum(), query.getPageSize());
...
@Override
@Transactional(rollbackFor = Exception.class)
public int updateByWrapper(Administrator obj,Wrapper wrapper){
...
LambdaUpdateWrapper<Administrator> updateWrapper = new UpdateWrapper<Administrator>().lambda();
updateWrapper.eq(Administrator::getId, id);
    ...
    public int batchDelList(List<Long> ids){
        ...
        */
        ...
        List<Long> idLists =Arrays.stream(ids.split(StringPool.COMMA)).map(s ->
            ...
            LambdaUpdateWrapper<Administrator> updateWrapper = new UpdateWrapper<Administrator>().lambda();
            updateWrapper.in(Administrator::getId, idLists);
            Administrator obj = new Administrator();
            obj.setState(Constant.STATE_0);
            return this.baseMapper.update(obj, updateWrapper);
            }
