...
public class FileUtil {
     ...
     */
    public static void download(String filePath, String fileName, boolean deleteFlag, HttpServletResponse response)
            throws IOException {
        ...
        if (!fileTypeIsValid(fileType)) {
            throw new FilerException("暂不支持该类型文件下载");
        }
    ...
    private static String getFileType(File file) throws FileNotFoundException {
        ...
        if (file.isDirectory()) {
            throw new FileNotFoundException("file不是文件");
        }
    ...
    /**
     * 校验文件类型是否是允许下载的类型 （出于安全考虑：https://github.com/wuyouzhuguli/aps-Shiro/issues/40）
     *
