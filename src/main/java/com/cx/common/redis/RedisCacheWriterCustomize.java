...
/**
 * 缓存写入实现
 * 重新DefaultRedisCacheWriter
 * 主要修改put 方法
 * @author Administrator
 * customize
 */
public class RedisCacheWriterCustomize implements RedisCacheWriter {
     ...
     */
    public RedisCacheWriterCustomize(RedisConnectionFactory connectionFactory) {
     ...
     *                          to disable locking.
     */
    RedisCacheWriterCustomize(RedisConnectionFactory connectionFactory, Duration sleepTime) {
    ...
    public void put(String name, byte[] key, byte[] value, @Nullable Duration ttl) {
        ...
        execute(name, connection -> {
            ...
            int index = name.lastIndexOf(Constant.REDIS_EXPIRE_TIME_KEY);
            if (index > 0) {
                //取出对应的时间 1200 index + 1是还有一个=号
                String expireString = name.substring(index + 1 + Constant.REDIS_EXPIRE_TIME_KEY.length());
