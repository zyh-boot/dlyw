layui.extend({
    ...
    soulTable: 'lay/extends/soulTable'
}).define(['conf', 'view', 'api', 'jquery', 'table','soulTable'], function (exports) {
    POPUP_DATA = {};
    var conf = layui.conf;
    ...
    self.initView = function (route) {
        ...
        if(self.route.href.indexOf("http:")>=0){
            route.fileurl = self.route.href;
        }else{
    ...
    self.isImage = function (str) {
        return /^([hH][tT]{2}[pP]:\/\/|[hH][tT]{2}[pP][sS]:\/\/)(.*).(jpg|gif|bmp|png)$/.test(
            str
        )
    };
    self.popup = function (params) {
        ...
        if (self.isUrl(url) || self.isImage(url)) {
            params.type = 2;
            params.content = url;
    ...
    function getRoute(){
        self.route = layui.router();
        if(!self.route.href || !$.trim(self.route.href) || self.route.href.length==0){
            self.route.href=window.location.hash.replace("#","");
        }
    }
    ...
    // ajax sync请求
    self.getSync = function (url, params, success) {
        if (params) {
            params.invalidate_ie_cache = new Date();
        }
        $.ajax({
